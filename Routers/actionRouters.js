...
router.get('/', (req, res) => { // get with /api/action in front
  aM
    .get()
    .then(actions => {
      res.json(actions);
  ...
  aM
    ...
    .then(actions => {
      ...
      requestedAction
        ...
        : res.status(404).json({ message: 'Requested action does not exist.' }); //user error
    })
    .catch(error => {
...
router.post('/', (req, res) => { //actions are connected to project 1
